...
class DataMoex():
  ...
  def stocks_prices_all_period(self, ticker, load_from_file = False):
    file_path = "{}/{}.csv".format(self.__DATA_FOLDER__, ticker)
    data_by_dates = {}
    if os.path.exists(file_path):
      df_readed = pd.read_csv(file_path)
      if load_from_file:
        return df_readed
      data_by_dates = { df_readed.iloc[pos]['date'] :{ column:df_readed.iloc[pos][column] for column in df_readed.columns[2:] } for pos in range(len(df_readed) - 1) }
    days = 0
    ...
    while last_date != get_date(prices[0]):
      ...
      for row in prices:
        if row['@OPEN']:
          ...
          data_by_dates[date] = {
            'open': float(row['@OPEN']),
            'close': float(row.get('@CLOSE') or row.get('@LAST') or row.get('@MARKETPRICE')),
            'high': float(row['@HIGH']),
            'low': float(row['@LOW']),
          ...
          for bank in self.banks:
            try:
              data_by_dates[date]["key_rate_{}".format(bank.__class__.__name__)] = 0 #bank.key_rate_by(date)
            except:
      ...
      days += 90
      dates_exists = any([data_by_dates.get(minus_today(day)) for day in range(days, days + 6, 1)] )
      if dates_exists:
        try:
          if loaded_range:
            break
    ...
    yesterday_volume, today_volume = dataframe[-2:].volume.values
    if yesterday_volume == today_volume:
